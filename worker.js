...
function cleanMediaUrl(
  ...
  /*
   * A Glide value can sometimes contain Markdown after
   * another layer of quoting. Try unwrapMarkdown once more.
   */
  const secondPass =
    unwrapMarkdown(
      text
  ...
  /*
   * If the whole value is a URL, return it directly.
   */
  if (
    /^https?:\/\/[^\s<>"')]+$/i.test(
      text
  ...
  /*
   * Fallback for a value that contains a URL.
   */
  const urlMatch =
    text.match(
      /https?:\/\/[^\s<>"')]+/i
